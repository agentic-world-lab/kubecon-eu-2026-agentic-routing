...
// Init implements EmbeddingClassifierInitializer (no-op since models are initialized externally)
func (c *EmbeddingClassifier) Init() error {
	return nil
}
...
func (c *EmbeddingClassifier) findBestRule(candidateSimilarities map[string]float32) (string, float32, error) {
	...
	for i := range ruleScores {
		if float64(ruleScores[i].score) >= c.optimizationConfig.MinScoreThreshold {
