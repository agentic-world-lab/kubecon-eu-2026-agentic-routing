# spain-electricity-cost-mcp

spain-electricity-cost-mcp is a Model Context Protocol (MCP) server built with FastMCP featuring dynamic tool loading.
...
2. **Run in Container**:
   ```bash
   docker run -i spain-electricity-cost-mcp:latest
   ```
...
kubectl get mcpserver spain-electricity-cost-mcp
```