...
openrouter-pricing-mcp is a Model Context Protocol (MCP) server built with FastMCP featuring dynamic tool loading.
...
2. **Run in Container**:
   ```bash
   docker run -i openrouter-pricing-mcp:latest
   ```
...
kubectl get mcpserver openrouter-pricing-mcp
```