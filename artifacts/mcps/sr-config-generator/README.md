...
sr-config-generator is a Model Context Protocol (MCP) server built with FastMCP featuring dynamic tool loading.
...
2. **Run in Container**:
   ```bash
   docker run -i sr-config-generator:latest
   ```
...
kubectl get mcpserver sr-config-generator
```